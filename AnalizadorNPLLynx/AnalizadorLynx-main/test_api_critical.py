...
def test_api_queries():
    ...
    for i, test_case in enumerate(test_queries, 1):
        ...
        try:
            print(f"\n{i}. 🔍 Consulta: '{query}'")            # Hacer request a la API
            response = requests.post(
              ...
              if response.status_code == 200:
                ...
                if "resultado" in data and "fase_5_motor_recomendaciones" in data["resultado"]:
                    motor = data["resultado"]["fase_5_motor_recomendaciones"]
                    estrategia = motor.get("estrategia_usada", "unknown")
                    productos = motor.get("productos_encontrados", [])
                    total = motor.get("total_encontrados", 0)
                elif "recommendations" in data and "interpretation" in data:
                    # New API structure
                    motor = {
                        "estrategia_usada": data["interpretation"].get("estrategia_usada", "unknown"),
                        "productos_encontrados": data["recommendations"],
                        "total_encontrados": len(data["recommendations"])
                    }
                    estrategia = motor["estrategia_usada"]
                    productos = motor["productos_encontrados"]
                    total = motor["total_encontrados"]
                else:
                ...
                # Verificar si hay productos esperados
                if productos:
                    productos_nombres = [p.get("name", p.get("nombre", "")) for p in productos[:3]]
                    print(f"   🎯 Productos:")
                    for j, producto in enumerate(productos[:3], 1):
                        nombre = producto.get("name", producto.get("nombre", "Sin nombre"))
                        precio = producto.get("price", producto.get("precio", 0))
                        stock = producto.get("stock", producto.get("cantidad", 0))
                        imagen = producto.get("image", producto.get("imagen", "default.jpg"))
